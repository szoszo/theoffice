...
describe("goalsForPrompt (operator goals layer, issue #21 §3)", () => {
  ...
  it("returns '' when GOALS.md is absent (pure no-op for tenants that don't use it)", () => {
    expect(goalsForPrompt(cfg(root))).toBe("");
  });

  it("returns '' for an empty / whitespace-only GOALS.md", () => {
    writeFileSync(join(root, "GOALS.md"), "   \n\n  ");
    expect(goalsForPrompt(cfg(root))).toBe("");
  });

  it("frames the goals under a prioritize-against header and preserves the content", () => {
  ...
  it("respects the absolute cap on an oversized GOALS.md (truncates with a marker)", () => {
  ...
  it("YIELDS (returns '') when the given budget leaves no room for a meaningful block", () => {
...
describe("firstMessagePreamble — goals + memory share ONE pane-inject cap", () => {
  ...
  it("GUARDRAIL: near-cap memory + full GOALS.md -> combined <= cap, memory intact, goals truncated", () => {
    ...
    const pre = firstMessagePreamble(cfg(root), "agent-x", "hi");
  ...
  it("EXTREME: memory nearly fills the cap -> goals yields entirely, memory intact, combined <= cap", () => {
  ...
  it("with room: BOTH goals (framing, on top) and memory, combined <= cap", () => {
  ...
  it("no memory + no GOALS.md -> '' (nothing injected)", () => {
    recallMock.value = "";
    expect(firstMessagePreamble(cfg(root), "agent-x", "hi")).toBe("");
  });
});
