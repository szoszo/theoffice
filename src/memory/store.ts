...
export function saveMemory(a: SaveMemoryArgs): number {
  ...
  // Live embed-on-save is OPT-IN and OFF by default. A synchronous embed on every save keeps
  // OLLAMA_KEEP_ALIVE perpetually reset on the shared box, so bge-m3 (~1.1G) never unloads. With this
  // off, a save writes no vector and never wakes the model; the daily backfill (embed-cli backfill via
  // office-embed-backfill.timer) vectorizes at 04:00 while the model is loaded only for that window.
  // Set OFFICE_EMBED_ON_SAVE=1 to restore synchronous live embedding (reversible). Either way the save
  // is fire-and-forget and never awaited: the memory is the asset, the vector is a recoverable
  // enhancement — a missing vector is filled by backfillEmbeddings(), a lost memory is not.
  if (embedOnSaveEnabled()) void attachEmbedding(id, embeddableText(a.content, a.keywords));
  return id;
}

/** Live embed-on-save: OFF unless OFFICE_EMBED_ON_SAVE=1. Read per-call so it is togglable without
 *  a code change, and so tests can flip it. Default off keeps bge-m3 unloaded between daily backfills. */
function embedOnSaveEnabled(): boolean {
  return process.env.OFFICE_EMBED_ON_SAVE === "1";
}
...
export function countEmbeddings(): {
  ...
  usable: number;
  /** NULL rows the embedder can NEVER fill: embeddableText is empty (content AND keywords both empty).
   *  Excluded from the coverage DENOMINATOR so one empty-content memory cannot read as permanently
   *  degraded (which would also starve the backfill-success marker). Reported separately for visibility.
   *
   *  CANONICAL RULE (kept in sync by hand across two languages): un-embeddable iff content AND keywords are
   *  both empty — the complement of embeddableText's "embeddable iff content OR keywords non-empty". The
   *  watchd check tenant/store/watchd-checks/memory_embedding_health.py applies the SAME rule via its
   *  UNEMBEDDABLE_SQL constant (its age-gate query and its own unembeddable count both derive from it), and
   *  fires a tripwire the day boundary rows appear so the deferred cross-language consistency test
   *  (card ae747288) gets built to pin these two definitions together. Change the rule in BOTH places. */
  unembeddable: number;
} {
  const r = getDb()
    .prepare(
      `SELECT COUNT(*) total,
              ...
              SUM(embedding IS NULL AND trim(content) = '' AND trim(COALESCE(keywords,'')) = '') unembeddable
         FROM memories`
    )
    .get(EXPECTED_DIM) as {
    total: number;
    ...
    unembeddable: number | null;
  };
  const total = Number(r.total ?? 0);
  const embedded = Number(r.embedded ?? 0);
  const wrongDim = Number(r.wrongDim ?? 0);
  const unembeddable = Number(r.unembeddable ?? 0);
  return { total, embedded, missing: total - embedded, wrongDim, usable: embedded - wrongDim, unembeddable };
}
