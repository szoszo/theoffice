...
describe("shipped prompt-layer invariants (issue #21 §4b)", () => {
  it("every shipped template/persona is tenant-agnostic AND mutating tasks keep their guardrails", async () => {
    expect(validateShippedPromptLayer(repoRoot)).toEqual([]);
  });

  it("the denylist actually contains the identifiers that leaked before (marveen, Szoszo)", async () => {
...
describe("checkTenantLeaks", () => {
  ...
  it("does NOT false-positive an agent-id embedded in an unrelated word", async () => {
  ...
  it("passes clean tenant-agnostic text", async () => {
...
describe("checkMutatingTaskSafety", () => {
  ...
  it("passes a prompt that retains every guardrail", async () => {
    expect(checkMutatingTaskSafety("t", safe)).toEqual([]);
  });
  it("flags a prompt missing the never-delete rule", async () => {
  ...
  it("flags a prompt missing the card-text-is-data hardening", async () => {
  ...
  it("flags a propose-not-destroy framing stripped to a bare 'propose' (no vacuous pass)", async () => {
...
describe("checkTaskConfigNoHardcodedAgent", () => {
  it("flags a task-config that hardcodes an agent", async () => {
    expect(checkTaskConfigNoHardcodedAgent("t", JSON.stringify({ schedule: "0 8 * * *", agent: "marveen" })).length).toBe(1);
  });
  it("passes an agent-less task-config (defaults to mainAgentId)", async () => {
...
vi.mock("../memory/recall.js", () => ({
  ...
  recallForPromptAsync: async (...a: unknown[]) => (() => h.mem)(...(a as [])),
}));
...
describe("shipped GOALS.md.example — preamble budget lock", () => {
  ...
  it("the shipped example is short (locked against bloat)", async () => {
  ...
  it("real example + near-cap memory keeps the combined preamble <= 6000, memory intact", async () => {
    ...
    try {
      ...
      const pre = await firstMessagePreamble(cfg, "agent-x", "hi");
  ...
  it("with room, the real example is surfaced under the goals header", async () => {
    const tenantRoot = mkdtempSync(join(tmpdir(), "goals-ex-"));
    try {
      copyFileSync(examplePath, join(tenantRoot, "GOALS.md"));
      const cfg = { paths: { tenantRoot } } as any;
